          ...
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <div style={{ fontSize: 12, color: 'var(--text-3)', whiteSpace: 'nowrap' }}>De:</div>
                <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
                  style={{ flex: 1, background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px', color: 'var(--text)', fontSize: 13, outline: 'none', cursor: 'pointer' }} />
                <div style={{ fontSize: 12, color: 'var(--text-3)', whiteSpace: 'nowrap' }}>Até:</div>
                <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)}
                  style={{ flex: 1, background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px', color: 'var(--text)', fontSize: 13, outline: 'none', cursor: 'pointer' }} />
                {(dateFrom || dateTo) && (
                  <button onClick={() => { setDateFrom(''); setDateTo('') }} style={{ background: 'var(--bg-3)', border: '1px solid var(--border)', borderRadius: 6, color: 'var(--text-3)', cursor: 'pointer', fontSize: 12, padding: '6px 10px', whiteSpace: 'nowrap' }}>Limpar</button>
                )}
              </div>
            </div>
                      ...
                      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                        ...
                        {b.status !== 'concluido' && (
                          <button onClick={() => sendReminder(b)} disabled={sendingReminder === b.id}
                            ...
                            {sendingReminder === b.id ? '...' : reminderSent === b.id ? '✓ Enviado' : '🔔'}
                          </button>
                        )}
                        {/* Copy link */}
